...
const AddNewMedication = props => {
    ...
    const data1 = [ "Apples", "Tomatoes", "Shoes", "Shopping"];
...
// MedDoseages.filter(x => x !== undefined)
    ...
    const getmedAPIData = async(text) => {
       // setStageLoaded(true);         MOVED TO TOUCHABLE OPACITY IN AUTOCOMPLETE
        setLoading(true);
        setMedNames([]);
        setMedDoseages([]);
        try {
    ...
    const [getDataNow, setGetDataNow] = useState(false);
    ...
    const getDosageData = async() => {
        ...
        if(getDataNow) {
            try {
                ...
                console.log("DOSE DATA      " + DOSEDATAJSON)
               ...
               var DOSES = [];
                ...
                DOSES = (DATAJSON[2].STRENGTHS_AND_FORMS[1][2]);
                setNEWDOSES(DOSES);
        ...
        <View>
            ...
            <View>
                ...
                <View style={styles.container, {marginTop:250}}>
                    ...
                    {stageLoaded && getDosageData() && // setGetDataNow(true) &&
