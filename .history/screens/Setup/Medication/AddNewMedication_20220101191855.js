...
const AddNewMedication = props => {
    ...
    var [MedNAMEData, setMedNAMEData] = useState([]);
    ...
    const getMedData = async() => {
        try {
            ...
            setMedNAMEData(json.map((jsonArray(json))));
    ...
    useEffect(() => {
        ...
        //     .then((data) => setMedNAMEData(JSON.parse(JSON.stringify(data))))
    ...
    function componentDidMount() {
         var account_nam=[]
                fetch(`https://clinicaltables.nlm.nih.gov/api/rxterms/v3/search?terms=${apiMed}&ef=DISPLAY_NAME,STRENGTHS_AND_FORMS`, {
                    method: 'GET',
                    headers: { 'Authorization': 'Bearer ' + 'DISPLAY_NAME' }
                })
        ...
        <View>
            ...
            <View>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => {
                        return(
                            <Text>{MedNAMEData.DISPLAY_NAME}</Text>
                        );
                    }}
                />
