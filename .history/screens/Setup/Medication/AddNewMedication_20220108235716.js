...
//    // console.log(":-----------------------------------------------------------------------------------------:");
    ...
    const getmedAPIData = async(text) => {
        ...
        setMedDoseages([]);
        try {
            const response = await fetch(`https://clinicaltables.nlm.nih.gov/api/rxterms/v3/search?maxList=6&terms=${text}&ef=DISPLAY_NAME,STRENGTHS_AND_FORMS`);
