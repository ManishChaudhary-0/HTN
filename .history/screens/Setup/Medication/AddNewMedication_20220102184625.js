...
const AddNewMedication = props => {
  ...
  //  console.log(data3)
    ...
    filtered = data3[0].replace(/[^0-9\.]+/g, " ").trim().split(" ").map(e => parseFloat(e));
   // console.log(filtered);
        ...
        try {
           ...
           // console.log(JSON.parse(JSON.stringify(json.DISPLAY_NAME[0])));
            ...
            for(let i = 0; i < apiMedsFound; i++){
                setMedNames()
                setMedNames().push(medAPIData[2].DISPLAY_NAME[i]);
                setMedDoseages().push(medAPIData[2].STRENGTHS_AND_FORMS[i])
    ...
    useEffect(() => {
        ...
        //     .then((data) => setmedAPIData(JSON.parse(JSON.stringify(data))))
